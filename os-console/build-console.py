...
(function () {
  ...
  function docsUrl() {
    ...
    if (!docsBlobUrl) {
      ...
      // Relay: intercept article-link clicks and dispatch hashchange explicitly so
      // the docs site's route() fires even when the browser silently skips the event
      // inside a blob-URL iframe loaded from a file:// page (Chrome/Edge known gap).
      var relay = '<script>(function(){'
        + 'document.addEventListener("click",function(e){'
        + 'var a=e.target.closest(\'a[href^="#/"]\');if(!a)return;'
        + 'var p=a.getAttribute("href");'
        + 'var parts=p.slice(2).split("/").filter(Boolean);'
        + 'if(parts.length!==2)return;'
        + 'e.preventDefault();history.pushState(null,"",p);'
        + 'window.dispatchEvent(new Event("hashchange"));'
        + '},true);'
        + '})();<\/script>';
      var html = LITE.docsHtml.indexOf('</title>') >= 0
        ? LITE.docsHtml.replace('</title>', '</title>' + style + relay)
        : LITE.docsHtml + style + relay;
